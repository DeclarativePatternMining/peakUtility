...
public final class PropPeakUtility extends Propagator<BoolVar> {
    ...
    public void propagate(int evtmask) throws ContradictionException {
        ...
        // -----------------------------------------------------------------
        // Rule 1: cover(x^-1(1)) empty => failure
        if (oneCount > 0 && proj.size() == 0) {
            fails();
            return;
        }

        // Rule 2: for each free i, if cover(x^-1(1) U {i}) empty => remove 1
        ...
        // Rule 3: TWU(x^-1(1)) < theta => failure
        ...
        long uOnes = exactUtilityOnProjection(proj, stateOrder, oneCount);

        // Rule 4: UB = u(ones) + ru(ones, frees)
        ...
        // Rule 5: for each free i, UB_i < theta => remove 1
        for (int ui = oneCount; ui < zeroStart; ui++) {
            int item = stateOrder[ui];
            TransactionProjection proj2 = proj.copy();
            proj2.intersectWithItem(data, item);
            if (proj2.size() == 0) {
                // already handled by Rule 2, but safe to keep
                vars[item].instantiateTo(0, this);
                continue;
            }

            long ruSi = remainingUtilityBoundOverFreesExcept(proj2, oneCount, zeroStart, item);
            long uItem = utilityOfItemOnProjection(proj2, item);
            long ub_i = uOnes + uItem + ruSi;
            if (ub_i < minUtil) {
                vars[item].instantiateTo(0, this);
            }
        }

        // If already fully assigned, apply peak utility checks (Rules 6--7)
        if (freeCount == 0) {
            if (uOnes < minUtil) {
                fails();
                return;
            ...
            if (mode == Mode.UPI) {
                applyPeakUtilityChecks(proj, stateOrder, oneCount, uOnes);
            }
            return;
        }
        ...
        if (mode == Mode.UPI && depthK > 0) {
            ...
            partitionStates();
            applyDepthKSubsetPruning(uOnes);
        }
    ...
    private long remainingUtilityBoundOverFreesExcept(TransactionProjection proj, int freeStart, int freeEnd, int excludedItem) {
        long ru = 0L;
        for (int i = freeStart; i < freeEnd; i++) {
            int freeItem = stateOrder[i];
            if (freeItem == excludedItem) {
                continue;
            }
            ru += utilityOfItemOnProjection(proj, freeItem);
        }
