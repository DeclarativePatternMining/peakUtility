...
if [ -z "${BASH_VERSION:-}" ]; then
  ...
  echo "Use: bash ./scripts/run_rule8_k_activation.sh ..." >&2
  exit 1
fi

# Count how many times Rule 8 fires (prunes) for different k values.
#
# Usage:
#   bash ./scripts/run_rule8_k_activation.sh <dataset> <minUtilAbs> <timeoutSecPerRun> [mode] [rulesMask] [varHeuristic] [valHeuristic] [kListCsv]
#
# Example:
#   bash ./scripts/run_rule8_k_activation.sh foodmart_utility_spmf 10000 60 UPI 11111111 TWU_DESC MIN "1,2,3,5,10,20"
#
# Output:
#   datasets/results/rule8_k_activation/<dataset>/rule8_k_activation.csv
#   datasets/results/rule8_k_activation/<dataset>/rule8_k_activation.log
#   datasets/results/rule8_k_activation/<dataset>/rule8_k_activation_table.tex

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"

if [ "$#" -lt 3 ] || [ "$#" -gt 8 ]; then
  echo "Usage: bash ./scripts/run_rule8_k_activation.sh <dataset> <minUtilAbs> <timeoutSecPerRun> [mode] [rulesMask] [varHeuristic] [valHeuristic] [kListCsv]"
  exit 1
fi
...
OUT_DIR="$ROOT_DIR/datasets/results/rule8_k_activation/$DATASET_KEY"
mkdir -p "$OUT_DIR"

CSV_FILE="$OUT_DIR/rule8_k_activation.csv"
LOG_FILE="$OUT_DIR/rule8_k_activation.log"
RUN_OUT_DIR="$OUT_DIR/runs"
TEX_FILE="$OUT_DIR/rule8_k_activation_table.tex"
mkdir -p "$RUN_OUT_DIR"
...

{
  ...
  echo "\\begin{table}[ht]"
  echo "\\centering"
  echo "\\caption{Rule 8 activation count for ${ds_tex} (minUtil=${MINUTIL}, timeout=${TIMEOUT_SEC}s, mode=${MODE}, rules=${RULES_MASK}, var=${VAR_HEUR}, val=${VAL_HEUR}).}"
  echo "\\begin{tabular}{rrrrr}"
  echo "\\hline"
  echo "$k$ & Rule8 fires & Itemsets & Nodes & Time (s) \\\\"
  echo "\\hline"

  tail -n +2 "$CSV_FILE" | while IFS=',' read -r _run_ts k rule8_triggers itemsets _timeout nodes solver_time _mode _rules _var _val; do
